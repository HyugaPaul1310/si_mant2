...
async function alterarEstadoEnum() {
  try {
    ...
    const query = `ALTER TABLE reportes MODIFY COLUMN estado ENUM('pendiente', 'asignado', 'en_proceso', 'en_cotizacion', 'cotizado', 'aceptado_por_cliente', 'finalizado_por_tecnico', 'cerrado_por_cliente', 'listo_para_encuesta', 'encuesta_satisfaccion', 'terminado', 'finalizado', 'en_espera', 'en_espera_confirmacion', 'rechazado') DEFAULT 'pendiente'`;

    console.log('[ALTER-ENUM] Ejecutando query:', query);
    await pool.query(query);

    console.log('[ALTER-ENUM] ✅ Tabla reportes alterada exitosamente');
    console.log('[ALTER-ENUM] El nuevo estado "en_espera_confirmacion" está disponible');
